...
A sandbox is a secure runtime environment for AI agents. Each sandbox provides an isolated [filesystem](/docs/filesystem), [shell access](/docs/commands), and [internet connectivity](/docs/sandbox/internet-access)—giving your agents the same capabilities they'd have on a real computer, without the security risks.

Sandboxes start in ~100ms and support long-lived sessions with full state [persistence](/docs/sandbox/persistence). You can pause a sandbox, resume it later, and pick up exactly where you left off.

The following example shows the core workflow: create a sandbox, run a command, and clean up. This pattern forms the foundation for more complex agent interactions—once you have a sandbox, you can run commands, read/write files, install packages, and more.
